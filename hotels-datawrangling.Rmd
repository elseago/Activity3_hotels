...
The most common number of adults is 2. There are some bookings for way more adults than fit in one room.
...
All the bookings for 6 or more people were canceled.

**Note:** Don't forget to label your R chunk as well (where it says `label-me-2`). 

```{r AdultNumFreqTableCancel}
##cancelled bookings
hotels %>% 
...
hotels %>% 
  ...
  select(arrival_date_month, arrival_date_year, children, babies, adults, adr, hotel)
```
